...
let enemies: string[] = ["Skeleton", "Zombie", "Warrior", "Assessin"];
let enemyMaxHealth: number = 75;
let enemyAttackDamage: number = 25;

// variable for player

let playerMaxHealth: number = 100;
let playerAttackDamage: number = 50;
let numHealthPotions: number = 3;
let healthPotionHealAmount: number = 30;
let healthPotionDropAmount: number = 50; // percentage
...
GAME: while (running) {
  ...
  let enemyHealth: number = Math.floor(Math.random() * enemyMaxHealth);
  ...
  while (enemyHealth > 0) {
    ...
    if (action === "A") {
      ...
      console.log(
        `\t> you recieve ${damageTaken} amount of damage in retaliation!`
      );

      if (playerMaxHealth < 1) {
        console.log(
          `\t> you have recieved to much damage, you are to weak to go on!`);
          break;
      }
    } else if (action === "D") {
      if (numHealthPotions > 0) {
        ...
        console.log(
          `\tYou dont have a health potion , Defeat the enimies for a chance to get one !`
        );
      }
    } else if (action === "R") {
